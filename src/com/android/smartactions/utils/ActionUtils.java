        ...
        if (uri.startsWith(ActionHandler.SYSTEM_PREFIX)) {
            for (int i = 0; i < ActionHandler.systemActions.length; i++) {
                if (ActionHandler.systemActions[i].mAction.equals(uri)) {
                    int resId = ActionHandler.systemActions[i].mLabelRes;
                    if (resId == 0) {
                        return "SmartAction Label Fail!";
                    }
                    return ctx.getResources().getString(ActionHandler.systemActions[i].mLabelRes);
                }
            }
        } else {
            try {
            ...
            for (int i = 0; i < ActionHandler.systemActions.length; i++) {
                if (ActionHandler.systemActions[i].mAction.equals(action)) {
                    return context.getResources().getDrawable(
                            ActionHandler.systemActions[i].mIconRes, context.getTheme());
                }
            }
        } else {
